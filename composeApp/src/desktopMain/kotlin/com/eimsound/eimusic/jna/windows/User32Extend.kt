...
// 监听主题变化
fun handleThemeChange(window: Window, isDarkMode: Boolean = true) {
    try {
        ...
        dwmApi?.getFunction("DwmSetWindowAttribute")?.apply {
            ...
            invoke(
                ...
                arrayOf(windowHandle, 20, IntByReference(if (isDarkTheme) 1 else 0), 4)
            )
        }
    } catch (e: Exception) {
        println("Failed to set dark mode: ${e.message}")
    }
}